...
bool Titan::setEnable() {
  ...
  return VMX::CAN::GetInstance().sendMessage(HG_ENABLE_FLAG, data,
                                                        5);
}
...
uint32_t Titan::getTitanWorld1() {
  // setEnable();
  VMX::CAN::GetInstance().createCANReceive("word1",
                                                      HG_RETURN_WORD_1);
  VMX::CAN::ReceiveStreamInfo info;
  sleep(1);
  info = VMX::CAN::GetInstance().readReceiveStream("word1");
  int32_t word1 = 0;
  word1 = info.messages_.data[0] + (info.messages_.data[1] << 8) +
          (info.messages_.data[2] << 16) + (info.messages_.data[3] << 24);
  // printf("%08X - ", word1);
  return word1;
}
uint32_t Titan::getTitanWorld2() {
  ...
  int32_t word2 = 0;
  word2 = info.messages_.data[0] + (info.messages_.data[1] << 8) +
          (info.messages_.data[2] << 16) + (info.messages_.data[3] << 24);
  // printf("%08X - ", word1);
  return word2;
}
uint32_t Titan::getTitanWorld3() {
  ...
  int32_t word3 = 0;
  word3 = info.messages_.data[0] + (info.messages_.data[1] << 8) +
          (info.messages_.data[2] << 16) + (info.messages_.data[3] << 24);
  // printf("%08X - ", word1);
  return word3;
}
std::string Titan::getTitanWorld() {
  return std::to_string(getTitanWorld1()) + std::to_string(getTitanWorld2()) +
         std::to_string(getTitanWorld3());
}
...
Motor::Motor(uint8_t index, uint16_t frequency) : m_index_(index) {
  ...
  if (!VMX::CAN::GetInstance().sendMessage(request_message_id,
                                                      data)) {
...
bool Motor::setSpeedAndDir(uint8_t speed, bool dir0, bool dir1) {
  ...
  if (!VMX::CAN::GetInstance().sendMessage(request_message_id,
                                                      data)) {
    std::cout << "Set Titan motor" << m_index_ << " speed and dir failed"
              << std::endl;
    return false;
  }
  return true;
...
TitanQuandLimit::TitanQuandLimit() {
  VMX::CAN::GetInstance().createCANReceive("TitanQuandLimit", HG_LIMIT_SWITCH);
}
