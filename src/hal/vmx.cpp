...
bool HicurrdioSupportOutput() {
  ...
  uint8_t num_hicurrdio_input_channels = VMXPI::GetInstance().io.GetNumChannelsByType(
      VMXChannelType::HiCurrDIO, first_hicurrdio_channel_index);
  bool supports_output = VMXPI::GetInstance().io.ChannelSupportsCapability(
      first_hicurrdio_channel_index, VMXChannelCapability::DigitalOutput);
  if (supports_output) {
...
float getAccFullScaleVoltage() {
  ...
  //  LOCK_GUARD(mtx);
  if (VMXPI::GetInstance().io.Accumulator_GetFullScaleVoltage(full_scale_voltage,
                                                 &errcode)) {
    return full_scale_voltage;
  }
  std::cout << "ERROR acquiring Analog Input Voltage."
            << GetVMXErrorString(errcode) << std::endl;
  return -1;
}

float getRoll() {

  return VMXPI::GetInstance().getAHRS().GetRoll();
}
float getPitch() {

  return VMXPI::GetInstance().getAHRS().GetPitch();
}
float getYaw() {

  return VMXPI::GetInstance().getAHRS().GetYaw();
}
void zeroYaw() {

  VMXPI::GetInstance().getAHRS().ZeroYaw();
}

float getCompassHeading() {

  return VMXPI::GetInstance().getAHRS().GetCompassHeading();
}
bool isCalibrating() {

  return VMXPI::GetInstance().getAHRS().IsCalibrating();
}
bool isConnected() {

  return VMXPI::GetInstance().getAHRS().IsConnected();
}
double getByteCount() {

  return VMXPI::GetInstance().getAHRS().GetByteCount();
}
double getUpdateCount() {

  return VMXPI::GetInstance().getAHRS().GetUpdateCount();
}
long getLastSensorTimestamp() {

  return VMXPI::GetInstance().getAHRS().GetLastSensorTimestamp();
}
float getWorldLinearAccelX() {

  return VMXPI::GetInstance().getAHRS().GetWorldLinearAccelX();
}
float getWorldLinearAccelY() {

  return VMXPI::GetInstance().getAHRS().GetWorldLinearAccelY();
}
float getWorldLinearAccelZ() {

  return VMXPI::GetInstance().getAHRS().GetWorldLinearAccelZ();
}
bool isMoving() {

  return VMXPI::GetInstance().getAHRS().IsMoving();
}
bool isRotating() {

  return VMXPI::GetInstance().getAHRS().IsRotating();
}
float getBarometricPressure() {

  return VMXPI::GetInstance().getAHRS().GetBarometricPressure();
}
float getAltitude() {

  return VMXPI::GetInstance().getAHRS().GetAltitude();
}
bool isAltitudeValid() {

  return VMXPI::GetInstance().getAHRS().IsAltitudeValid();
}
...
void updateDisplacement(float accel_x_g, float accel_y_g,
  ...
  VMXPI::GetInstance().getAHRS().UpdateDisplacement(accel_x_g, accel_y_g, update_rate_hz,
                                       is_moving);
}
float getVelocityX() {

  return VMXPI::GetInstance().getAHRS().GetVelocityX();
}
float getVelocityY() {

  return VMXPI::GetInstance().getAHRS().GetVelocityY();
}
float getVelocityZ() {

  return VMXPI::GetInstance().getAHRS().GetVelocityZ();
}
float getDisplacementX() {

  return VMXPI::GetInstance().getAHRS().GetDisplacementX();
}
float getDisplacementY() {

  return VMXPI::GetInstance().getAHRS().GetDisplacementY();
}
float getDisplacementZ() {

  return VMXPI::GetInstance().getAHRS().GetDisplacementZ();
}
double getAngle() {

  return VMXPI::GetInstance().getAHRS().GetAngle();
}
double getRate() {

  return VMXPI::GetInstance().getAHRS().GetRate();
}
void reset() {

  VMXPI::GetInstance().getAHRS().Reset();
}

float getRawGyroX() {

  return VMXPI::GetInstance().getAHRS().GetRawGyroX();
}
float getRawGyroY() {

  return VMXPI::GetInstance().getAHRS().GetRawGyroY();
}
float getRawGyroZ() {

  return VMXPI::GetInstance().getAHRS().GetRawGyroZ();
}
float getRawAccelX() {

  return VMXPI::GetInstance().getAHRS().GetRawAccelX();
}
float getRawAccelY() {

  return VMXPI::GetInstance().getAHRS().GetRawAccelY();
}
float getRawAccelZ() {

  return VMXPI::GetInstance().getAHRS().GetRawAccelZ();
}
float getRawMagX() {

  return VMXPI::GetInstance().getAHRS().GetRawMagX();
}
float getRawMagY() {

  return VMXPI::GetInstance().getAHRS().GetRawMagZ();
}
float getRawMagZ() {
...
float getPressure() {
  ...
  return VMXPI::GetInstance().getAHRS().GetPressure();
}
float getTempC() {

  return VMXPI::GetInstance().getAHRS().GetTempC();
}
...
bool Channel::Deactivate() {
  //  LOCK_GUARD(mtx);
  bool rt = VMXPI::GetInstance().io.DeactivateResource(
      m_resource_handle, &m_errcode);
  if (!rt) {
    std::cout << "Deactivate false"
              << GetVMXErrorString(m_errcode) << std::endl;
  }
  rt = VMXPI::GetInstance().io.DeallocateResource(
      m_resource_handle, &m_errcode);
  if (!rt) {
    std::cout << "Deallocate false"
              << GetVMXErrorString(m_errcode) << std::endl;
  }
  return rt;
}
...
bool SingleChannel::Activate(const VMXResourceConfig& res_cfg) {
  //  LOCK_GUARD(mtx);
  bool rt =
      VMXPI::GetInstance().io.ActivateSinglechannelResource(
          m_info, &res_cfg, m_resource_handle, &m_errcode);
  if (!rt) {
    std::cout << "Active IO" << m_info.index << " false"
              << GetVMXErrorString(m_errcode) << std::endl;
  }
  return rt;
}
...
bool DualChannel::Activate(const VMXResourceConfig& res_cfg) {
  //  LOCK_GUARD(mtx);
  bool rt = VMXPI::GetInstance().io.ActivateDualchannelResource(
      m_info[0], m_info[1], &res_cfg, m_resource_handle, &m_errcode);
  return rt;
}
...
bool QuadChannel::Activate(const VMXResourceConfig& res_cfg) {
  //  LOCK_GUARD(mtx);
  bool rt = VMXPI::GetInstance().io.ActivateQuadchannelResource(
      m_info[0], m_info[1], m_info[2], m_info[3], &res_cfg, m_resource_handle,
      &m_errcode);
  return rt;
}
...
int32_t ENC::read() {
  //  LOCK_GUARD(mtx);
  bool rt = VMXPI::GetInstance().io.Encoder_GetCount(
      m_resource_handle, m_counter_, &m_errcode);
  if (!rt) {
    std::cout << "ENC:" << (m_info[0].index) / 2 << " read failed"
              << GetVMXErrorString(m_errcode);
  }
  return m_counter_;
}
bool ENC::reset() {
  //  LOCK_GUARD(mtx);
  bool rt = VMXPI::GetInstance().io.Encoder_Reset(
      m_resource_handle, &m_errcode);
  if (!rt) {
    std::cout << "ENC:" << (m_info[0].index) / 2 << " reset failed"
              << GetVMXErrorString(m_errcode);
  }
  m_counter_ = 0;
  return rt;
...
bool DI::read() {
  bool rt = VMXPI::GetInstance().io.DIO_Get(m_resource_handle,
                                                           m_sigal, &m_errcode);
  if (!rt) {
...
PWM::PWM(uint8_t index, uint32_t frequency,
         PWMGeneratorConfig::FrameOutputFilter output_filter,
...
void PWM::setDutyCycle(double duty_cycle) {
  ...
  bool rt = VMXPI::GetInstance().io.PWMGenerator_SetDutyCycle(
      m_resource_handle, 0, m_duty_cycle, &m_errcode);
  if (!rt) {
    std::cout << "PWM:" << m_info.index << " set PWM duty cycle failed"
              << std::endl;
...
CAN::CAN(int32_t period_ms, VMXCAN::VMXCANMode mode) : m_period_ms_(period_ms) {
  ...
  bool rt =
      VMXPI::GetInstance().can.SetMode(mode, &m_errcode_) &&
      VMXPI::GetInstance().can.ResetBusBitrate(
          VMXCAN::CANBusBitrate::CAN_BUS_BITRATE_1MBPS, &m_errcode_);
  if (!rt) {
    std::cout << "VMX CAN"
              << " init failed."
...
CAN& CAN::GetInstance(int32_t period_ms,
...
void CAN::flushRxFIFO() {
  ...
  if (!VMXPI::GetInstance().can.FlushRxFIFO(&m_errcode_)) {
    std::cout << "Flush Rx FIFO"
              << " fail." << GetVMXErrorString(m_errcode_)
              << std::endl;
  }
}
...
bool CAN::sendMessage(uint32_t SendMessageID, std::vector<uint8_t> data,
                      int32_t period_ms) {
  m_period_ms_ = 1;
  ...
  // {
  //  LOCK_GUARD(mtx);
  bool rt = VMXPI::GetInstance().can.SendMessage(
      msg, m_period_ms_, &m_errcode_);
  if (!rt) {
    std::cout << "SendMessageID:" << SendMessageID << " send message fail"
              << " message info : " << printMsgInfo(msg) << "."
              << GetVMXErrorString(m_errcode_);
  }
  // }
...
bool CAN::sendMessage(uint32_t SendMessageID, uint8_t data[8],
                      int32_t period_ms) {
...
bool CAN::createCANReceive(std::string receive_stream_name,
                           uint32_t receive_message_id) {
  ...
  bool rt = VMXPI::GetInstance().can.OpenReceiveStream(
      receive_stream_info.can_rx_handles_,
      receive_stream_info.recvice_message_id_,
      receive_stream_info.message_mask_, receive_stream_info.max_messages_,
      &m_errcode_);
  if (!rt) {
    std::cout << "Open receive stream:"
              << receive_stream_info.recvice_message_id_ << " open fail,"
              << GetVMXErrorString(m_errcode_) << std::endl;
    return false;
  }
  rt = VMXPI::GetInstance().can.EnableReceiveStreamBlackboard(
      receive_stream_info.can_rx_handles_, true, &m_errcode_);
  if (!rt) {
    std::cout << "Enable receive stream:"
              << receive_stream_info.recvice_message_id_ << " open fail"
              << GetVMXErrorString(m_errcode_) << std::endl;
    return false;
  }
  m_receive_stream_info_.insert(std::pair<string, ReceiveStreamInfo>(
      receive_stream_name, receive_stream_info));
  // }
  return rt;
}
CAN::ReceiveStreamInfo CAN::readReceiveStream(std::string receive_stream_name) {
  ...
  if (iter == m_receive_stream_info_.end()) {
    std::cout << "Can't find receive stream:" << receive_stream_name
              << std::endl;
    return no_info;
  }
  // {
  ...
  bool rt = VMXPI::GetInstance().can.ReadReceiveStream(iter->second.can_rx_handles_
  , &iter->second.messages_, 1,
  ...
  if (!rt) {
    ...
    std::cout << "Read receive stream:" << iter->second.recvice_message_id_
              << " failed"
              << "receive stream is name:" << receive_stream_name << "."
              << GetVMXErrorString(m_errcode_);
    return no_info;
  }
...
std::vector<uint8_t> CAN::readReceiveStreamData(
    std::string receive_stream_name) {
...
VMXIIC::VMXIIC(uint8_t register_address) : m_register_address_(register_address) {
  m_info[0] = VMXChannelInfo(VMXPI::GetInstance().getIO().GetSoleChannelIndex
            (VMXChannelCapability::I2C_SDA), VMXChannelCapability::I2C_SDA);
  m_info[1] = VMXChannelInfo(VMXPI::GetInstance().getIO().GetSoleChannelIndex
            (VMXChannelCapability::I2C_SCL), VMXChannelCapability::I2C_SCL);
  Activate(m_config_);
}
std::vector<uint8_t> VMXIIC::IICTransaction(std::vector<uint8_t> p_send_data, uint16_t receiveSize) {
  ...
  bool rt = VMXPI::GetInstance().io.I2C_Transaction(m_resource_handle, m_register_address_, p_send_data_ptr, p_send_data.size(),
                            p_rcv_data_ptr, receiveSize, &m_errcode);
  m_success_ = rt;
  if (!rt) {
    std::cout << "IIC" << " transaction failed" << std::endl;
    std::cout << GetVMXErrorString(m_errcode) << std::endl;
    return p_rcv_data;
  }
...
ADS1115::ADS1115() {
  m_iic_ = std::make_shared<VMXIIC>(0x48);
...
float ADS1115::get(uint8_t index) {
  return m_channel_[index];
}
...
uint32_t Timer::read() {
  bool rt = VMXPI::GetInstance().io.InputCapture_GetChannelCounts(m_resource_handle, m_ch1_count_, m_ch2_count_, &m_errcode);
  if (!rt) {
