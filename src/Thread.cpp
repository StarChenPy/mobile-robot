...
Thread::Thread(std::function<void()> cb, const std::string& name)
  ...
  if (rt) {
    // HGSYS_LOG_ERROR(g_logger) << "pthread_create thread fail, rt= " << rt
    //   << "name=" << name;
   ...
   * @brief 等待线程已启动
   * 
   * @details 在run函数中有信号量notify，正确且等待启动一个线程后才会退出线程初始化
   *          开始下一个线程的初始化
   * 
   */
  m_semaphore_.wait();
...
void Thread::join() {
  if (m_thread_) {
    ...
    if (rt) {
    //   HGSYS_LOG_ERROR(g_logger) << "pthread_join thread fail, rt=" << rt
    //     << "name=" << m_name_;
