...
Timer::Timer(uint64_t ms, Command::Ptr command, bool recurring,
             TimerManager *manager)
    : m_ms_(ms),
      m_command_(command),
      m_recurring_(recurring),
      m_manager_(manager) {
...
Timer::Ptr TimerManager::addTimer(uint64_t ms, Command::Ptr command,
                                  bool recurring) {
...
void TimerManager::handleExpiredTimers(
    ...
    std::set<RobotGenius::Timer::Ptr, RobotGenius::Timer::Comparator>::iterator
        &it) {
  expired.clear();
  RWMutexType::ReadLock lock(m_mutex_);
  if (m_timers_.empty()) {
...
// {
  ...
  for (auto &timer : expired) {
    ...
    // if (timer->m_command_->getWorkCommandState() == Command::State::RUNNING) {
