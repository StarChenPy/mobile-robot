...
ConditionalComamand::ConditionalComamand(std::function<bool()> condition, CommandBase::Ptr on_true_command, CommandBase::Ptr on_false_command)
...
void ConditionalComamand::execute() {
  m_work_command_->execute();
}
void ConditionalComamand::end() {
  m_work_command_->end();
}

bool ConditionalComamand::isFinished() {
