...
void AlongRightWallCommand::execute() {
  ...
  if(!lidar_data.empty()){
    ...
    // Robot::GetInstance().lidar_calib->LidarAlongRightWallTask(speed, lidar_data, calib_d);
...
void AlongLeftWallCommand::execute() {
  ...
  if(!lidar_data.empty()){
    ...
    // Robot::GetInstance().lidar_calib->LidarAlongLeftWallTask(speed, lidar_data, calib_d);
    double R_setpoint = Robot::GetInstance().lidar_calib->get_R_setpoint();
    double L_setpoint = Robot::GetInstance().lidar_calib->get_L_setpoint();
...
bool isReachXCommand::isFinished() {
  return is_finished;
}
