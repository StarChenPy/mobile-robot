...
InterruptibleCommand::InterruptibleCommand(Command::Ptr primary_command, Command::Ptr interrupt_condition, Command::Ptr fallback_command) {
...
void InterruptibleCommand::execute() {
  ...
  switch (m_status) {
    case Status::BEFORE:
      if (m_primary_command_->m_state == State::HOLDON) m_primary_command_->m_state = State::RUNNING;
      if (m_primary_command_->getTimer().get()) m_primary_command_->getTimer()->setPause(false);
      else {
        // if (m_primary_command_->m_state == State::WAIT)
        m_primary_command_->schedule();
      } 
      if (m_interrupt_condition_->getTimer().get()) m_interrupt_condition_->getTimer()->setPause(false);
      else  {
    ...
    case Status::InterruptB:
      ...
      if (m_primary_command_->getTimer().get()) m_primary_command_->getTimer()->setPause();
      if (m_interrupt_condition_->isFinisheddec()) {
        m_status = Status::Interrupt;
      }
      break;
    case Status::Interrupt:
    if (m_interrupt_condition_->getTimer().get()) m_interrupt_condition_->getTimer()->setPause();
...
void InterruptibleCommand::end() {
  ...
  if (!m_primary_command_->isFinisheddec())  m_primary_command_->cancel();
  if (!m_interrupt_condition_->isFinisheddec() ) m_interrupt_condition_->cancel();
  if (!m_fallback_command_->isFinisheddec()
      && m_fallback_command_->m_state != State::WAIT) m_fallback_command_->cancel();
