...
Command::Ptr createTrackingPointCommand(Pose target_pose, double v) {   //第二个参数是设定速度
  return std::make_shared<TrackingPointCommand>(target_pose, v)->withTimer(100);
}
Command::Ptr createTrackingPointCommand(Pose target_pose, double v, double e_dis, double e_phi) {   //第二个参数是设定速度
...
Command::Ptr createTrackingVectorCommand(const vector<Pose>& points, double v) {  //第二个参数是设定速度
  SequentialCommandGroup::Ptr S = std::make_shared<SequentialCommandGroup>();
  for(int i = 0; i < points.size(); i++){
...
Command::Ptr createTrackingXYCommand(Pose target_pose, double v) {  //第二个参数是设定速度
  return std::make_shared<TrackingXYCommand>(target_pose, v)->withTimer(100);
}
Command::Ptr createTrackingXYCommand(Pose target_pose, double v, double e_dis, double e_phi) {   //第二个参数是设定速度
...
Command::Ptr createTrackingVectorXYCommand(const vector<Pose>& points, double v) {  //第二个参数是设定速度
  SequentialCommandGroup::Ptr S = std::make_shared<SequentialCommandGroup>();
  for(int i = 0; i < points.size(); i++){
