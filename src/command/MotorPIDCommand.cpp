...
void LeftMotorPIDCommand::initialize() { is_finished = false; }
...
void TurnMotorPIDCommand::initialize() { is_finished = false; }
...
void LiftMotorPIDCommand::initialize() { is_finished = false; }
...
Command::ptr createLeftMotorPIDCommand() { return std::make_shared<LeftMotorPIDCommand>()->withTimer(20); }
Command::ptr createRightMotorPIDCommand() { return std::make_shared<RightMotorPIDCommand>()->withTimer(20); }
Command::ptr createTurnMotorPIDCommand() { return std::make_shared<TurnMotorPIDCommand>()->withTimer(20); }
Command::ptr createLiftMotorPIDCommand() { return std::make_shared<LiftMotorPIDCommand>()->withTimer(20); }

LeftMotorCommand::LeftMotorCommand() {}

void LeftMotorCommand::initialize() { is_finished = false; }

void LeftMotorCommand::execute() { Robot::GetInstance().setLeftMotorSpeedWithoutPID(); }

void LeftMotorCommand::end() { return; }
...
void RightMotorCommand::initialize() { is_finished = false; }

void RightMotorCommand::execute() { Robot::GetInstance().setRightMotorSpeedWithoutPID(); }

void RightMotorCommand::end() { return; }
...
void TurnMotorCommand::initialize() { is_finished = false; }

void TurnMotorCommand::execute() { Robot::GetInstance().setTurnMotorSpeedWithoutPID(); }

void TurnMotorCommand::end() { return; }
...
void LiftMotorCommand::initialize() { is_finished = false; }

void LiftMotorCommand::execute() { Robot::GetInstance().setLiftMotorSpeedWithoutPID(); }
...
void LiftMotorDistancePIDCommand::initialize() {
    ...
    // Robot::instance().resetLiftMotorPID();
    // Robot::instance().reset_lift_enc();
}
...
Command::ptr LiftDistancePIDCommand(double h) {
    ...
    LiftMotorDistancePIDCommand::Ptr Lift_Motor_Distance_PID_Command =
        std::make_shared<LiftMotorDistancePIDCommand>(setpoint);
...
Command::ptr LiftDistancePIDCommand(double h, double e_enc, uint8_t cnt) {
    ...
    int32_t setpoint = static_cast<int32_t>(h * (1000 / 10.7 / 2));
    LiftMotorDistancePIDCommand::Ptr Lift_Motor_Distance_PID_Command =
        std::make_shared<LiftMotorDistancePIDCommand>(setpoint, e_enc, cnt);
    return Lift_Motor_Distance_PID_Command->withTimer(100);
}
...
void ResetLiftMotorDistancePIDCommand::execute() {
    ...
    if (!finished) {
        // std::cout << "TurningLimit->read() " << TurningLimit->read() <<
        // std::endl;
...
Command::ptr ResetLiftMotorDistance(int32_t speed) {
    ResetLiftMotorDistancePIDCommand::Ptr reset_lift_command =
        std::make_shared<ResetLiftMotorDistancePIDCommand>(speed);
    return reset_lift_command->withTimer(100);
}

void ResetLiftMotorDistancePIDCAssistance::initialize() { m_current_counter = 0; }
...
void TurnMotorDistancePIDCommand::initialize() {
    ...
    // Robot::instance().setTurnMotorDistance(set_point_);
    // Robot::instance().reset_turn_enc();
}
...
Command::ptr TurnAnglePIDCommand(double angle) {
    ...
    TurnMotorDistancePIDCommand::Ptr Turn_Motor_Distance_PID_Command =
        std::make_shared<TurnMotorDistancePIDCommand>(setpoint);
...
Command::ptr TurnAnglePIDCommand(double angle, double e_enc, uint8_t cnt) {
    ...
    int32_t setpoint = static_cast<int32_t>(angle * (1670.0 / 90.0) - 2);
    TurnMotorDistancePIDCommand::Ptr Turn_Motor_Distance_PID_Command =
        std::make_shared<TurnMotorDistancePIDCommand>(setpoint, e_enc, cnt);
    return Turn_Motor_Distance_PID_Command->withTimer(100);
}
...
void ResetTurnMotorDistancePIDCommand::initialize() {
    ...
    // Robot::instance().resetTurnMotorPID();
    // Robot::instance().reset_turn_enc();
}
...
void ResetTurnMotorDistancePIDommandAssistance::initialize() { m_current_counter = 0; }
...
Command::ptr ResetTurnMotorDistance(int32_t setpoint) {
    ResetTurnMotorDistancePIDCommand::Ptr reset_turn_command =
        std::make_shared<ResetTurnMotorDistancePIDCommand>(setpoint);
    // readTurnENCCommand::ptr read_turn_enc_command =
    // std::make_shared<readTurnENCCommand>(); TurnMotorPIDCommand::ptr
...
void SetRightMotorSpeedCommand::end() {
    // Robot::instance().setRightMotorSpeed(0);
}

bool SetRightMotorSpeedCommand::isFinished() { return Robot::GetInstance().getStopSignal() || is_finished; }
...
bool SetLeftMotorSpeedCommand::isFinished() { return Robot::GetInstance().getStopSignal() || is_finished; }
