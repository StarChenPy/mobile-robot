...
void LabviewCommand::TrackingPointShareCommand(){  //坐标移动
  if(TrackingPointStatusShare == COMMEND_CREATE){
    ...
    createTrackingPointCommand(target, TrackingPointShare.params.v, TrackingPointShare.params.point_err, TrackingPointShare.params.angle_err)->schedule();
  }
}
void LabviewCommand::TrackingXYShareCommand(){  //坐标移动XY 
  if(TrackingXYStatusShare == COMMEND_CREATE){
    ...
    LABVIEW::TrackingPointShareAddress->read(TrackingPointShare);
    Pose target(TrackingPointShare.pose.x_, TrackingPointShare.pose.y_, TrackingPointShare.pose.theta_);
    createTrackingXYCommand(target, TrackingPointShare.params.v, TrackingPointShare.params.point_err, TrackingPointShare.params.angle_err)->schedule();
  }
}
...
void LabviewCommand::execute() {
  ...
  // std::cout << "LabviewCommand execute dt = " << static_cast<double>(dt) << std::endl;
...
Command::Ptr createLabviewCommand(){
  return std::make_shared<LabviewCommand>()->withTimer(100);
}
