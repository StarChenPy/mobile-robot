...
void ZeroOdomCommand::execute() {
  ...
  // std::cout << "ZeroOdomCommand execute dt = " << static_cast<double>(dt) << std::endl;
  // is_finished = true;
}
...
Command::Ptr createZeroOdomCommand(){
  return std::make_shared<ZeroOdomCommand>()->withTimer(100);
}
...
void SetOdomCommand::execute() {
  ...
  // std::cout << "SetOdomCommand execute dt = " << static_cast<double>(dt) << std::endl;
  // is_finished = true;
}
