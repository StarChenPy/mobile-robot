...
void IRCalCommand::execute() {
  double error = Robot::GetInstance().calibrationIR(m_distance, m_angle_error, m_disatance_error, m_left_right_e, m_offset);
  if (!error) {
    m_current_counter = 0;
  } else {
    m_current_counter++;
...
Command::Ptr IRCalCommandAssistance(double distance) {
  IRCalCommand::Ptr command = std::make_shared<IRCalCommand>(distance, IRCalibE.Angle, IRCalibE.Dis, IRCalibE.CNT, IRCalibE.LeftRightE, 0);
  return command->withTimer(100);
}
Command::Ptr IRCalCommandAssistance(double distance, double angle_error, double disatance_error, uint32_t counter, double left_right_e, double offset) {
  IRCalCommand::Ptr command = std::make_shared<IRCalCommand>(distance, angle_error, disatance_error, counter, left_right_e, offset);
  return command->withTimer(100);
}
...
Command::Ptr SingleIRCalCommandAssistance(double distance) {
  SingleIRCalCommand::Ptr command = std::make_shared<SingleIRCalCommand>(distance, IRCalibE.Angle, IRCalibE.Dis, IRCalibE.CNT);
  return command->withTimer(100);
}
Command::Ptr SingleIRCalCommandAssistance(double distance, double angle_error, double disatance_error, uint32_t counter) {
  SingleIRCalCommand::Ptr command = std::make_shared<SingleIRCalCommand>(distance, angle_error, disatance_error, counter);
  return command->withTimer(100);
}
...
void USCalCommand::execute() {
  double error = Robot::GetInstance().calibrationUS(m_distance, m_angle_error, m_disatance_error, m_left_right_e, m_offset);
  if (!error) {
    m_current_counter = 0;
  } else {
    m_current_counter++;
...
Command::Ptr USCalCommandAssistance(double distance) {
  USCalCommand::Ptr command = std::make_shared<USCalCommand>(distance, USCalibE.Angle, USCalibE.Dis, USCalibE.CNT, USCalibE.LeftRightE, 0);
  return command->withTimer(100);
}
Command::Ptr USCalCommandAssistance(double distance, double angle_error, double disatance_error, uint32_t counter, double left_right_e, double offset) {
  USCalCommand::Ptr command = std::make_shared<USCalCommand>(distance, angle_error, disatance_error, counter, left_right_e, offset);
  return command->withTimer(100);
}