...
void SequentialCommandGroup::execute() {
  ...
  if (!m_current_command_) return;
  if (m_current_command_->isFinished()) {
    
    if (m_current_command_index_ + 1 < m_commands_.size()) {
        // std::cout << "m_current_command_index_" << m_commands_.size() << " " << m_current_command_index_ << " " << m_current_command_ << std::endl;
        m_current_command_index_++;
        m_current_command_ = m_commands_[m_current_command_index_];
        // std::cout << "m_current_command_index_" << m_current_command_index_ << " " << m_current_command_ << std::endl;
        // m_current_command_->m_state = Command::State::INIT;
...
void SequentialCommandGroup::end() {
  ...
  // for (int index = m_current_command_index_; index < m_commands_.size(); index++) {
