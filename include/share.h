...
template <class T> class ShareMemory {
  public:
...
template <class T> ShareMemory<T>::ShareMemory(int key) : key_(key) {
    std::cout << "createSharedMemory: " << createSharedMemory(key_) << std::endl;
}
template <class T> ShareMemory<T>::~ShareMemory() { destroySharedMemory(); }
template <class T> bool ShareMemory<T>::createSharedMemory(int key) {
    /// @brief 创建内存信号量
    sem_ = new Semaphore(key_);
    /// 尝试打开已存在的共享内存
    shm_id_ = shmget(key_, sizeof(T), 0);
    if (shm_id_ == -1) {
        // 队列状态共享内存不存在，创建共享内存
        shm_id_ = shmget(key_, sizeof(T), IPC_CREAT | 0666);
        if (shm_id_ == -1) {
            return false;
        }
    }
    // 连接共享内存
    buffer_ = static_cast<T *>(shmat(shm_id_, nullptr, 0));
    if (buffer_ == nullptr) {
        return false;
    }
    return true;
}
template <class T> bool ShareMemory<T>::destroySharedMemory() {
    if (buffer_ != nullptr) {
        // 解除连接共享内存
        if (shmdt(buffer_) == -1) {
            return false;
...
template <class T> void ShareMemory<T>::read(T &data) {
    sem_->P();
    data = *buffer_;
    sem_->V();
}
template <class T> void ShareMemory<T>::write(T data) {
    sem_->P();
    *buffer_ = data;
    sem_->V();
}
template <class T> void ShareMemory<T>::getSharedMemoryStatus(int &shm_id, int &key) {
    shm_id = shm_id_;
    key = key_;
}
template <class T> void ShareMemory<T>::printStatus() {
    std::cout << "shared_memory shm_id:" << shm_id_ << " key:" << key_ << std::endl;
}

template <typename T> class SharedMemoryArray {
  public:
    typedef std::shared_ptr<SharedMemoryArray<T>> ptr;
    SharedMemoryArray(int key, int rows) : rows_(rows), sharedMemory_(NULL), key_(key) {
        /// @brief 创建内存信号量
        /// @brief 创建内存信号量
        sem_ = new Semaphore(key_);
        ...
        if (shm_id_ == -1) {
            ...
            if (shm_id_ == -1) {
                std::cerr << "Failed to get existing shared memory segment." << std::endl;
            }
        }
...
struct BoxInfo {
    ...
    BoxInfo(float x1, float y1, float x2, float y2, float score, int32_t label)
...
struct FruitsInfo { //多个水果
    ...
    FruitsInfo() = default;
    FruitsInfo(uint8_t num, const IdentifyInfo &fruits_1, const IdentifyInfo &fruits_2, const IdentifyInfo &fruits_3,
               const IdentifyInfo &fruits_4, const IdentifyInfo &fruits_5)
        : num(num), fruits_1(fruits_1), fruits_2(fruits_2), fruits_3(fruits_3), fruits_4(fruits_4), fruits_5(fruits_5) {
    }
    FruitsInfo(const FruitsInfo &temp)
        : num(temp.num), fruits_1(temp.fruits_1), fruits_2(temp.fruits_2), fruits_3(temp.fruits_3),
          fruits_4(temp.fruits_4), fruits_5(temp.fruits_5) {}
};
