...
class UpdateOdom {
 public:
  ...
  double calWhellSpeed(double enc_process, double period){
    // std::cout << "len2grating_k = " << len2grating_k << " enc_process = " << enc_process << " (1000 / period) = " << (1000 / period) << std::endl;
  ...
  void UpdateWhellData(double L_emc, double R_enc, double period){
    ...
    // std::cout << "Whell_R.enc_process = " << Whell_R.enc_process << " Whell_L.enc_process = " << Whell_L.enc_process << std::endl;
  }
