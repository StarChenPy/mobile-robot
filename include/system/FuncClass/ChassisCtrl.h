...
class ChassisCtrl {
  public:
    ...
    CarCtrlVal PurePursuit(const Pose &target, const Pose &cur) {
        ...
        double delta_d =
            sqrt(targetRobotCoorPoint.x_ * targetRobotCoorPoint.x_ + targetRobotCoorPoint.y_ * targetRobotCoorPoint.y_);
        // double l_d = 200;
        ...
        // 到达判定
        if (std::fabs(delta_d) < delta_d_max ||
            ((std::fabs(ld_x) < (2 / 3) * delta_d_max) &&
             (std::fabs(ld_y) < (5 / 2) * delta_d_max))) //当相对距离小于某个值判定为到达
        {
            // reachPoint_cnt++;
        ...
        // std::cout << "v = " << ctrl_val.v << " w = " << ctrl_val.w  <<
        // std::endl; std::cout << "car(" << cur.x_ << ", " << cur.y_ << ") -> "
        //           << "PathPoint(" << target.x_ << ", " << target.y_ << "),
        //           angle = " << cur.theta_ << std::endl;
    ...
    void CtrlConstraint() {
        ...
        //最大最小值约束
        if (CtrlValInput.v > V_MAX) {
            CtrlValInput.v = V_MAX;
        } else if (CtrlValInput.v < V_MIN && CtrlValInput.v > -V_MIN) {
            CtrlValInput.v = 0;
        }
        // else if(CtrlValInput.v < V_MIN){CtrlValInput.v = 0;}
        if (CtrlValInput.w > W_MAX) {
            CtrlValInput.w = W_MAX;
        } else if (CtrlValInput.w < -W_MAX) {
            CtrlValInput.w = -W_MAX;
        } else if (CtrlValInput.w < W_MIN && CtrlValInput.w > -W_MIN) {
            CtrlValInput.w = 0;
        }
        ...
        if (std::isnan(CtrlValInput.v) || std::isnan(CtrlValInput.w)) {
            CtrlValInput.v = 0;
            CtrlValInput.w = 0;
            // std::cout << "CtrlConstraint v = "<< CtrlValInput.v << " w = "<<
            // CtrlValInput.w << std::endl;
        }
        //保存上一次控制量数值
    ...
    //轮子速度转编码器速度
    double Speed2m_setpoint(double v_speed) { return Speed2m_setpoint_k * v_speed; }
    ...
    bool RotateTask(double target_PHi, double curPHi) {
        ...
        // std::cout << "target_PHi = " << target_PHi << ", curPHi = " << curPHi
        // << std::endl;
        CtrlConstraint();
        InverseKinematicsFunc(CtrlValInput.v, CtrlValInput.w);
        Whell_setpoint();
        // std::cout << "Whell_R.setpoint = " << Whell_R.setpoint << ",
        // Whell_L.setpoint = " << Whell_L.setpoint << std::endl;
    ...
    bool VisionCtrlTask(int fruit_cx, int cap_cx) {
        ...
        std::cout << "fruit_cx = " << fruit_cx << " cap_cx_ = " << cap_cx << " setpoint = " << setpoint << std::endl;
  ...
  private:
    ...
    double Speed2m_setpoint_k; //轮子运动速度对于光栅速度的比值, set_point_ = k
                               //* Speed
