  ...
  public:
    ...
    std::vector<Polar> Sampling(const vector<LidarData> &lidar_buf, double angle_min, double angle_max) {
        ...
        for (int i = 0; i < lidar_buf.size(); i++) {
            ...
            // cout << "Range = " << data.Range * 100 << " Angle = " <<
            // data.Angle * (180/M_PI) << " i = " << i << endl;
            if (data.Angle > angle_min * (M_PI / 180) && data.Angle < angle_max * (M_PI / 180)) {
                ...
                // cout << "Range = " << lidar_polar.range << " Angle = " <<
                // lidar_polar.angle << " i = " << i << endl;
            }
        }
    ...
    bool LidarCalib(std::vector<Polar> lidar_sample) {
        ...
        // std::cout << "abs(d_wall - (R_Setpoint+L_Setpoint)/2):" << abs(d_wall
        // - (R_L+ L_L)/2) << std::endl;
    ...
    double get_R_setpoint() { return R_Setpoint; }
    double get_L_setpoint() { return L_Setpoint; }
