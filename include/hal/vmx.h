...
class ENC : public DualChannel, std::enable_shared_from_this<ENC>{
 public:
  ...
  explicit ENC(
      uint8_t index,
      EncoderConfig::EncoderEdge edge = EncoderConfig::EncoderEdge::x4);
  ~ENC() {}
...
class DI : public SingleChannel, std::enable_shared_from_this<DI> {
 public:
   ...
   * @param index
   */
  explicit DI(uint8_t index,
              DIOConfig::InputMode input_mode = DIOConfig::InputMode::NONE);
  /**
   * @brief 析构函数
   *
   */
 ...
 public:
  ...
  explicit PWM(uint8_t index, uint32_t frequency = 50,
               PWMGeneratorConfig::FrameOutputFilter output_filter =
                   PWMGeneratorConfig::FrameOutputFilter::NONE,
...
class CAN : public std::enable_shared_from_this<CAN> {
 public:
  ...
  static CAN& GetInstance(int32_t period_ms = 0,
               VMXCAN::VMXCANMode mode = VMXCAN::VMXCANMode::VMXCAN_NORMAL);
  ...
  explicit CAN(int32_t period_ms = 0,
               VMXCAN::VMXCANMode mode = VMXCAN::VMXCANMode::VMXCAN_NORMAL);
  ~CAN() {}
  ...
  bool sendMessage(uint32_t SendMessageID, std::vector<uint8_t> data,
                   int32_t period_ms = 0);
  bool sendMessage(uint32_t SendMessageID, uint8_t* data,
                   int32_t period_ms = 0);
  bool createCANReceive(std::string receive_stream_name,
