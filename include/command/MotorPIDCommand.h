...
class LiftMotorDistancePIDCommand : public CommandBase {
  public:
    ...
    LiftMotorDistancePIDCommand(int32_t setpoint, double e_enc, uint8_t cnt)
        : m_setpoint(setpoint), E_ENC(e_enc), CNT(cnt) {}
    void initialize() override;
    void execute() override;
    void end() override;
    bool isFinished() override;
...
Command::ptr LiftDistancePIDCommand(double h, double e_enc = LIFTMOTORDISTANCEERROR,
                                    uint8_t cnt = LIFTMOTORDISTANCECounter);
...
class TurnMotorDistancePIDCommand : public CommandBase {
  public:
    ...
    TurnMotorDistancePIDCommand(int32_t setpoint, double e_enc, uint8_t cnt)
        : m_setpoint(setpoint), E_ENC(e_enc), CNT(cnt) {}
    void initialize() override;
    void execute() override;
    void end() override;
...
Command::ptr TurnAnglePIDCommand(double angle, double e_enc = TURNMOTORDISTANCEERROR,
                                 uint8_t cnt = TURNMOTORDISTANCECounter);
...
class ResetTurnMotorDistancePIDommandAssistance : public CommandBase {
  public:
    ...
    ResetTurnMotorDistancePIDommandAssistance(double speed = 0, int32_t counter = 10)
        : m_speed(speed), m_conter(counter) {}
    void initialize() override;
    void execute() override;
    void end() override;
    bool isFinished() override;
