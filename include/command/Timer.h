...
friend class Timer;
 public:
   ...
   * @return Timer::ptr
   */
  Timer::Ptr addTimer(uint64_t ms, std::shared_ptr<Command> command
 ...
 private:
   ...
   * 
   */
  void handleExpiredTimers(std::vector<Timer::Ptr>& expired, uint64_t now_ms, std::set<RobotGenius::Timer::Ptr, RobotGenius::Timer::Comparator>::iterator& it);
